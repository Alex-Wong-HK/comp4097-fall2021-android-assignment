...
@ExperimentalCoilApi
@Composable
fun QponFilterByMall(nav: NavHostController, restaurantId: String = "") {
    Scaffold(
        topBar = {
            TopAppBar(
                title = {Text( "Coupon List:("+restaurantId +")")},
                navigationIcon = {
    ...
    )
    {
        ...
        LazyColumn(modifier = Modifier.padding(start = 8.dp, end = 8.dp, bottom = 64.dp)) {
            items(data.filter { it.restaurant == restaurantId}, itemContent = { item ->
                ListItem( modifier = Modifier.clickable {
                    nav.navigate("couponDetail/${item.id}/${restaurantId}")
                }) {
